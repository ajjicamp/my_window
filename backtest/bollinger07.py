...
class BollingerTrader:
    ...
    def startTrader(self, table_list):
        ...
        for i, table in enumerate(table_list):
            ...
            df_day = df_day[buy_cond]

            print('소요시간', time.time() - start_time)
            # print(f"시물레이션 중 {table}... {i + 1} / {len(self.table_list)}")
            self.codeTrading(table, df_day, multiple)  # 종목별로 날짜를 달리하여 여러개의 deal이 있을 수 있다.

    def codeTrading(self, table, df_day, multiple):
